...
(last week)** to a fixed interval of 15 minutes. An automatic interval can use
wide time buckets for a week-long range, making a bar appear earlier than the
actual bell signal. Fifteen-minute buckets keep the graph readable while
placing normal school-bell times in the expected quarter hour.

The keyword range uses `Europe/Brussels`, which makes week boundaries follow
Belgian local time, including daylight-saving changes. School Bell, the Graylog
host and indexed event timestamps should continue using UTC internally.
...
3. Under the row grouping for `timestamp`, change **Interval** from **Auto**
   to **15 minutes**.
4. Open the widget-specific time range using the clock icon. Select
   **Keyword**, enter `This week`, and select `Europe/Brussels` as timezone.
5. Apply the time range and click **Update widget**.
6. Repeat for **Successful rings (last week)** with the keyword `Last week`
   and the same 15-minute interval.

The exact label or placement of the clock control can vary slightly by Graylog
version. The fixed interval controls aggregation; the timezone controls the
keyword range boundaries and display context. They solve separate problems.
